...
def delete(arr, obj, axis=0):
    ...
    if axis==1:
        ...
        for i in range(arr.shape[0]):
            ...
            new_data.append(new_row)
            new_row = []
    return array(new_data, dtype=arr.dtype)
...
class Array2D(Array):
    ...
    def get_row(self, i):
        new_data = []
        for j in range(self.shape[1]):
            new_data.append(self[i,j])
        return array([new_data], dtype=self.dtype)
    ...
    def delete(self, obj, axis):
        new_object = delete(self, obj, axis)
        self = new_object
        return self
